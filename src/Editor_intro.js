...
export default function Editor_intro () {
	...
	The following expectations should be known, before starting:
	    ...
	    <li>telephone clinics <strong>must</strong> have <code>longitude</code> set to <code>0</code> but do not need to have valid <code>postcode</code> (all co-ordinates are used to calculate distances, without exception, so omitted co-ordinates will cause the booking selection to crash, and there are also various checks for <code>0</code> elsewhere...)</li>
	    </ul>
